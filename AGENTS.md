...
npm run fmt         # Format
npm run lint        # Lint
npm run check       # Type-check (note: wasm bindings must be built first to pass app type-check)
```
...
The app imports the wasm engine as a normal workspace package (`import … from "wasm"`). The wasm package's `package.json` points at its generated `pkg/` output.
...
- `wasm/src/lib.rs` — the physics engine. `Simulation` stores grid, positions, and directions as flat `Vec`s in grid-space. Each tick: move → wall-bounce → cell-collision.
