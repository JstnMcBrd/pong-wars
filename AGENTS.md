...
- `app/src/canvas.ts` — `Canvas` singleton. Draws a `Frame` by writing the grid to a pixel-sized canvas and overlaying transparent ball strokes on a second canvas, keeping both layers in sync with the CSS-rendered size via a `ResizeObserver`.
